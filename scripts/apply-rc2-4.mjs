...
if (!app.includes('const [bankSavings, setBankSavings]')) {
  app = replaceOnce(app,
...
if (!app.includes('bankDetected = 0')) {
  app = replaceOnce(app,
...
if (!app.includes('Versements Belfius identifiés dans le CSV')) {
  app = replaceOnce(app,
...
if (!app.includes('bankDetected={bankSavings[goal.id]')) {
  app = replaceOnce(app,
...
if (!app.includes('onSavingsDetected={setBankSavings}')) {
  app = replaceOnce(app,
...
if (!audit.includes('  onSavingsDetected,')) {
...
if (!audit.includes('detectSavingsTransfers(parsedAudit.rows)')) {
  audit = replaceOnce(audit,
