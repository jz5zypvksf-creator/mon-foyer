...
import { buildRecurringHistoryPresentationRows } from './features/operations/recurringHistoryPresentation.js';
...
const ProtectedSettings = lazy(() => import('./features/security/components/ProtectedSettings.jsx'));
...
export default function App() {
  ...
  const recurringHistoryPresentationRows = useMemo(
    () => buildRecurringHistoryPresentationRows({
      ...
      selectedMonth,
      balanceCutoff,
      hasImportedCsv: Boolean(importedBelfiusAudit?.rows?.length),
    }),
    [balanceCutoff, data.recurringFixedExpenses, importedBelfiusAudit, monthOperations, outstandingRecurringExpenses, selectedMonth],
  );

  const historyMonthOperations = useMemo(
    () => [...monthOperations, ...outstandingRecurringExpenses, ...recurringHistoryPresentationRows]
