...
function belfiusAuditRc246Integration() {
  return {
    ...
    transform(code, id) {
      ...
      patched = patched.replace(".filter((row) => !isBeobankSavingsTransfer(row, savingsGoals))", ".filter((row) => !classifyBankBusinessRule(row, savingsGoals)?.excludeFromExpenseMatching)");
  ...
  return {
    ...
    transform(code, id) {
      ...
      if (isApp) {
        ...
        patched = patched.replace("  const totals = useMemo(() => {\n    return calculateTotals(effectiveMonthOperations, reimbursablePeople);\n  }, [effectiveMonthOperations, reimbursablePeople]);", "  const budgetIncomeTotal = useMemo(() => budgetIncomeTotalForMonth(data.operations, selectedMonth), [data.operations, selectedMonth]);\n\n  const totals = useMemo(() => { const actual = calculateTotals(effectiveMonthOperations, reimbursablePeople); return { ...actual, income: budgetIncomeTotal, balance: budgetIncomeTotal - actual.fixed - actual.variable }; }, [budgetIncomeTotal, effectiveMonthOperations, reimbursablePeople]);");
        patched = patched.replace("  const fullMonthTotals = useMemo(() => {\n    return calculateTotals(monthOperations, reimbursablePeople);\n  }, [monthOperations, reimbursablePeople]);", "  const fullMonthTotals = useMemo(() => { const actual = calculateTotals(monthOperations, reimbursablePeople); return { ...actual, income: budgetIncomeTotal, balance: budgetIncomeTotal - actual.fixed - actual.variable }; }, [budgetIncomeTotal, monthOperations, reimbursablePeople]);");
        patched = patched.replace("  const historyTotals = useMemo(() => {\n    const filteredTotals = calculateTotals(filteredMonthOperations, reimbursablePeople);\n    return {\n      ...filteredTotals,\n      expenses: filteredTotals.fixed + filteredTotals.variable,\n    };\n  }, [filteredMonthOperations, reimbursablePeople]);", "  const historyTotals = useMemo(() => { const filteredTotals = calculateTotals(filteredMonthOperations, reimbursablePeople); const defaultBudgetView = historyType === 'all' && historyPerson === 'all' && historyCategory === 'all' && historyPaymentMethod === 'all' && !historySearch.trim() && !showReviewOnly; const income = defaultBudgetView ? budgetIncomeTotal : filteredTotals.income; return { ...filteredTotals, income, balance: income - filteredTotals.fixed - filteredTotals.variable, expenses: filteredTotals.fixed + filteredTotals.variable }; }, [budgetIncomeTotal, filteredMonthOperations, historyCategory, historyPaymentMethod, historyPerson, historySearch, historyType, reimbursablePeople, showReviewOnly]);");
        ...
        if (!patched.includes('<option value="reimbursement">')) {
