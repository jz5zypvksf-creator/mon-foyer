...
function belfiusAuditRc246Integration() {
  return {
    ...
    transform(code, id) {
      ...
      const requiredMarkers = ["classifyBankBusinessRule(row)?.excludeFromExpenseMatching", "shouldOfferAmountDateFallback(bankRow, recurringExpenses)", "Écritures Mon Foyer sans mouvement Belfius", "sameAppIdentity", "__directDebitMatch"];
      if (requiredMarkers.some((marker) => !patched.includes(marker))) throw new Error('RC2.4.6 Belfius: integration target not found; refusing misleading build.');
      return { code: patched, map: null };
    },
  };
}
  ...
  return {
    ...
    transform(code, id) {
      ...
      if (isAudit) {
        ...
        patched = patched.replace("                <article key={row.id}><strong>{row.date} · {row.label}</strong><span>{money(row.type === 'income' ? row.amount : -row.amount)}</span></article>", "                <article key={row.id} className=\"audit-missing-row\"><strong>{row.date} · {row.label}</strong><span className=\"audit-missing-actions\"><b>{money(row.type === 'income' ? row.amount : -row.amount)}</b>{typeof onEditAppOperation === 'function' && (<button type=\"button\" className=\"audit-pencil\" title=\"Modifier cette écriture\" aria-label={`Modifier ${row.label}`} onClick={() => onEditAppOperation(row)}><Pencil size={17} /></button>)}</span></article>");
      }
